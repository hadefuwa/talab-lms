...
const SUBJECT_STYLES: Record<string, { bg: string; text: string; icon: string }> = {
  Maths:      { bg: "bg-blue-500",   text: "text-blue-600",   icon: "123" },
  Math:       { bg: "bg-blue-500",   text: "text-blue-600",   icon: "123" },
  English:    { bg: "bg-violet-500", text: "text-violet-600", icon: "Aa" },
  History:    { bg: "bg-amber-500",  text: "text-amber-600",  icon: "H" },
  Technology: { bg: "bg-teal-500",   text: "text-teal-600",   icon: "T" },
  Science:    { bg: "bg-green-500",  text: "text-green-600",  icon: "S" },
  Arabic:     { bg: "bg-rose-500",   text: "text-rose-600",   icon: "Ar" },
  Quran:      { bg: "bg-emerald-500",text: "text-emerald-600",icon: "Q" },
  Art:        { bg: "bg-pink-500",   text: "text-pink-600",   icon: "Art" },
  PE:         { bg: "bg-orange-500", text: "text-orange-600", icon: "PE" },
};
...
export default function CourseCard({ course, isFounder, hasAccess = true }: Props) {
  ...
  const empty = !isFounder && lessonCount === 0;
  const disabled = locked || empty;

  const cardClassName = `relative group bg-white border-2 rounded-2xl overflow-hidden transition-all duration-200 shadow-card ${
    disabled
  ...
  const content = (
    <div className={empty ? "block p-5 cursor-not-allowed" : "block p-5"}>
      <div className="flex items-start justify-between mb-4">
        <div className={`w-10 h-10 ${empty ? "bg-slate-300" : style.bg} rounded-xl flex items-center justify-center text-sm text-white font-black ${disabled ? "opacity-60" : ""}`}>
          {style.icon}
        </div>
        <div className="flex items-center gap-2">
          {empty && (
            <span className="text-xs text-slate-500 bg-slate-100 border border-slate-200 px-2 py-0.5 rounded-full font-bold">
              Coming soon
            </span>
          )}
          {!course.is_published && (
      ...
      <h3 className={`text-base font-bold leading-snug ${
        disabled ? "text-slate-400" : "text-slate-800 group-hover:text-talab-600"
      } transition-colors`}>
        {course.title}
      </h3>
      {course.description && (
      ...
      {empty ? (
        <div className="mt-3 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2">
          <p className="text-xs text-slate-500 font-semibold">No lessons available yet</p>
          <p className="text-xs text-slate-400 mt-0.5">This subject is coming soon.</p>
        </div>
      ) : lessonCount === 0 ? (
        ...
        <div className="mt-3 space-y-2">
          <div className="flex items-center justify-between text-xs text-slate-400">
            <span>{lessonCount} lesson{lessonCount !== 1 ? "s" : ""}
              {quizCount > 0 && ` | ${quizCount} quiz${quizCount !== 1 ? "zes" : ""}`}
            </span>
            <span className={`font-semibold ${completedCount > 0 ? style.text : "text-slate-300"}`}>
          ...
          <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden">
            <div
              ...
              style={{ width: `${lessonCount ? Math.round((completedCount / lessonCount) * 100) : 0}%` }}
            />
          </div>
        </div>
      )}
  ...
  return (
    <div className={cardClassName}>
      <div className={`h-2 w-full ${empty ? "bg-slate-300" : style.bg} ${disabled ? "opacity-40" : ""}`} />
      ...
      {empty ? (
        ...
        <Link href={locked ? "/billing" : `/courses/${course.id}`}>
          {content}
        </Link>
      )}
    </div>
