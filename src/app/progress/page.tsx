...
export default async function MyProgressPage() {
  ...
  const [
    ...
    supabase.from("progress_logs").select("*").eq("student_id", user.id),
    (supabase as any).from("quiz_attempts").select("*").eq("student_id", user.id).order("created_at", { ascending: false }),
  ]);
  ...
  return (
    <div className="min-h-screen bg-gray-950">
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 py-10">
        <a href="/dashboard" className="text-sm text-talab-500 hover:text-talab-400 inline-flex items-center gap-1 mb-6">
          ← Back to Dashboard
        </a>
