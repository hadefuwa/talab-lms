...
export default async function DashboardPage() {
  ...
  // Fetch lessons and progress for all courses
  ...
  if (courses.length > 0) {
    ...
    const completedIds = new Set(
      (progressRows ?? []).filter((r: { status: string }) => r.status === "completed").map((r: { lesson_id: string }) => r.lesson_id)
    );
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  <div className={`w-12 h-12 ${tier.bg} rounded-2xl flex items-center justify-center text-2xl shadow-sm`}>
                    {tier.icon}
                  </div>
                  {allDone && (
                ...
                <p className="text-xs text-slate-400 mt-1">{count} subject{count !== 1 ? "s" : ""}</p>
                ...
                <p className={`text-xs font-semibold mt-3 ${tier.text}`}>
                  Explore →
                </p>
              </Link>
            );
